...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    uiOutput("bordercity"),
    uiOutput("search"),
    selectInput("diagram", "Diagramtípus", c("Vonal", "Pont", "Kör", "Idősor", "Dekompozíciós modell"), selected = "Vonal"),
  ...
  navset_card_underline(id = "navset", 
                        ...
                        nav_panel("Tableau", gwalkrOutput("tableau")),
                        nav_spacer(),
...
server <- function(input, output, session) {
  ...
  subsetted <- reactive({
    ...
    if(input$navset != "Tableau" && (input$diagram == "Vonal" || input$diagram == "Pont" || input$diagram == "Kör")){
    ...
    if(input$navset != "Tableau"){
      ...
      tryCatch({
        ...
        if(input$diagram == "Vonal"){
          ...
          map <- ggplot(data = subsetted, aes(x = Dátum, y = Forgalom, colour = Honosság)) + geom_line() + labs(x = "Dátum", y = "Forgalom", colour = "Honosság") + facet_wrap(facet = vars(subsetted$Irány, subsetted$Jármű))
          ggplotly(map)
          
        }else if (input$diagram == "Pont"){
          
          colnames(subsetted) <- c("Irány", "Honosság", "Határátkelőhely", "Jármű", "Forgalom", "Dátum")
          map <- ggplot(data = subsetted, aes(x = Dátum, y = Forgalom, fill = Honosság)) + geom_point() + facet_wrap(facet = vars(subsetted$Irány, subsetted$Jármű))
          ggplotly(map)
